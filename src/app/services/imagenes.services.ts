...
export class ImagenesService {
    ...
    constructor(private db: AngularFirestore) {
        this.imagenesCollection = db.collection<ImagenesModel>('tbl-face'); //Este es el nombre de la tabla de la bdd no relacional
    }
    ...
    async guardarImagen(imagen: {
        ...
        try {
            return await this.db.collection('tbl-face').add(imagen);
        } catch (err) {
            console.log(err);
        }
    }
